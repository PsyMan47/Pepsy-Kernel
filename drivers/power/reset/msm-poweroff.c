...
static int warm_reset_set(const char *val, struct kernel_param *kp);
...
module_param_call(warm_reset, warm_reset_set, param_get_int,
			&warm_reset, 0644);
...
static int warm_reset_set(const char *val, struct kernel_param *kp)
{
	...
	/* If warm_reset is not zero or one, ignore. */
	if (warm_reset >> 1) {
		pr_err("%s: val is illegal\n", __FUNCTION__);
		return -EINVAL;
	}

	if (warm_reset) {
		pr_info("set to warm reset\n");
		qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);
	} else{
		pr_info("set to cold reset\n");
